...
import pairedSampleTTest from '@/assets/md/pairedSampleTTest.md';
...
export default {
        ...
        $route: {
            handler: function (route, oldRoute) {
                ...
                if (!docName) {
                    ...
                    // 切换文档
                    const menuItem = this.flatMenu.find((item) => item.name === docName);
    ...
    methods: {
        ...
        locationToAnchor() {
            ...
            if (anchor) {
                const target_dom = document.getElementById(anchor.replace(/^#/, ''));
                if (target_dom) {
        ...
        removeListenScroll() {
            ...
            scroll_dom && scroll_dom.removeEventListener('scroll', this.scrollHandle);
        },
        listenScroll() {
            const scroll_dom = document.querySelector('.markdown-body');
            scroll_dom && scroll_dom.addEventListener('scroll', this.scrollHandle);
        },
        scrollHandle() {
