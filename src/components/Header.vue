<template>
    ...
    >
        ...
        <wj-button type="primary" v-if="!isLogin" class="bg-blue" @click="openLoginDialog"
            >登录</wj-button
        >
        ...
        >
            <div class="flex flex-col items-center">
                ...
                <wj-form :model="form" class="px-16 mt-6" ref="loginForm" :rules="rules">
                    <wj-form-item prop="phone">
                        <wj-input v-model="form.phone" placeholder="请输入手机号码"></wj-input>
                    </wj-form-item>
                    ...
                    <wj-form-item prop="validateCode">
                        <div class="flex items-center">
                            ...
                            <wj-button
                                ...
                                @click="sendCode"
                                style="height: 38px"
                                >{{ cooling ? `${countDown}秒后获取` : '获取验证码' }}</wj-button
                            >
                        </div>
                    </wj-form-item>
                    ...
                    <span class="text-sm pl-1">
                        我接受<span @click="openUrl('/terms')" class="text-blue cursor-pointer"
                            >开发者协议</span
                        >及<span @click="openUrl('/privacy')" class="text-blue cursor-pointer"
                            >隐私政策</span
                        >
                    </span>
                    <wj-form-item class="mt-3">
                        <wj-button type="primary" class="bg-blue w-full h-10" @click="login"
                            >登录</wj-button
                        >
                        <p class="text-gray-500 text-xs text-center w-full mt-4">
                            未注册的手机号验证后自动注册
                        </p>
                    </wj-form-item>
                </wj-form>
...
export default {
    ...
    methods: {
        ...mapActions(['openLoginDialog', 'closeLoginDialog', 'changeLoginDialog']),
        login() {
            this.$refs['loginForm'].validate(async (valid) => {
                if (valid) {
                    ...
                    authorization && localStorage.setItem('auth_token', authorization);
